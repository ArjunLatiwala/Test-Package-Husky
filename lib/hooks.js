...
# Detection priority:
#   1. Local node_modules binary  (installed by cs-setup / npm install)
#   2. Global sonar-scanner on PATH
#   3. Auto-install via npx sonarqube-scanner (downloads on first use, cached after)
# This ensures the scanner is ALWAYS available on a fresh machine without any
# manual steps from the developer.
if [ -f "./node_modules/.bin/sonar-scanner" ]; then
  SONAR_BIN="./node_modules/.bin/sonar-scanner"
elif command -v sonar-scanner >/dev/null 2>&1; then
  SONAR_BIN="sonar-scanner"
else
  # Auto-install sonarqube-scanner locally so subsequent commits are faster
  echo "[SonarQube] sonar-scanner not found in node_modules — installing via npm..."
  npm install --save-dev sonarqube-scanner --quiet 2>/dev/null
  if [ -f "./node_modules/.bin/sonar-scanner" ]; then
    SONAR_BIN="./node_modules/.bin/sonar-scanner"
    echo "[SonarQube] sonarqube-scanner installed successfully."
  else
    # Last resort: use npx (downloads temporarily, no install required)
    echo "[SonarQube] Falling back to npx sonar-scanner..."
